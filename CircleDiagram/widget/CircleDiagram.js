...
mxui.widget.declare('CircleDiagram.widget.CircleDiagram', {
	...
	createLabel: function(knob){
		...
		fontSizeFromTo = Math.round(fontSizePercentage / 1.5);
		...
		dfromto.html(prefixType + Math.round(this.valueOriginal) + ' / ' + this.maxValueUsed);
	...
	refresh : function(objguid) {
		...
		if (this.mxobj.getGuid() == objguid) {
			...
			if(this.showPercentage){
				...
				this.descriptionNodeFromTo.html(Math.round(this.valueOriginal) + ' / ' + this.maxValueUsed);
